...
# from manage import s_username, s_email, s_password
# import manage
s_username = 'root'
...
class SuccessfulPasswordResetTests(TestCase):
    def setUp(self):
        ...
        url = reverse('password_reset')
        self.response = self.client.post(url, {'email': s_email})
...
class PasswordResetConfirmTests(TestCase):
    def setUp(self):
        user = User.objects.create_user(username=s_username, email=s_email, password=s_password)
        ...
        '''
        # self.uid = urlsafe_base64_encode(force_bytes(user.pk)).decode() <-- .decode() is predicated!!
